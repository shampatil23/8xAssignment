  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        {loading || !settings ? (
          <div className="bg-white rounded-xl border p-12 text-center text-xs text-slate-400">
          ...
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div>
                  <label className="font-bold text-slate-700 block mb-1">Store / Site Name</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="font-bold text-slate-700 block mb-1">Platform Currency</label>
                  <select
                    value={settings.currency}
                    onChange={(e) => setSettings({ ...settings, currency: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 bg-white focus:border-purple-500 focus:outline-hidden"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 focus:border-purple-500 focus:outline-hidden"
                  />
                  <p className="text-[10px] text-slate-400 mt-1">Default fee for non-prime orders.</p>
                </div>

                <div>
                  <label className="font-bold text-slate-700 block mb-1">Default Tax Rate (%)</label>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 focus:border-purple-500 focus:outline-hidden"
                  />
                  <p className="text-[10px] text-slate-400 mt-1">Applied during checkout calculation.</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-4">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-100">
                  <div>
                    <p className="font-bold text-slate-800">Auto-Approve Customer Reviews</p>
                ...
                <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 border border-slate-100">
                  <div>
                    <p className="font-bold text-slate-800">Allow New Customer Registrations</p>
                    <p className="text-slate-500 text-[11px]">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                  <span className="text-[10px] uppercase font-bold text-slate-400">Account Email</span>
                  <p className="font-bold text-slate-800 mt-0.5">{user?.email}</p>
                </div>
                <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                  <span className="text-[10px] uppercase font-bold text-slate-400">Role Verification</span>
                  <p className="font-bold text-purple-700 uppercase mt-0.5 flex items-center gap-1">
                    <ShieldCheck size={14} />
                    {user?.role}
                  </p>
                </div>
                <div className="p-3 bg-slate-50 rounded-lg border border-slate-100">
                  <span className="text-[10px] uppercase font-bold text-slate-400">Admin UID</span>
                  <p className="font-mono text-[11px] text-slate-600 mt-0.5 truncate">{user?.uid}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <Button
                ...
                size="md"
                isLoading={saving}
                className="inline-flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white font-bold"
              >
                <Save size={16} />
